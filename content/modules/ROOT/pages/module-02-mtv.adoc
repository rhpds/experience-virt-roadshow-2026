...
Documentation for how to install and configure the Operator can be found https://docs.redhat.com/en/documentation/migration_toolkit_for_virtualization/2.11/html/installing_and_using_the_migration_toolkit_for_virtualization/installing-the-operator_mtv[here^].
...
. On the next screen you will be tasked with providing networking details for your migration plan. Select the radio button to *Use a new network map*, then from the Source network dropdown select *segment-migration-to-ocpvirt* and for the Target network dropdown select the *Default network*. Finally, fill in the Network map name with *default-network-map* and click the *Next* button.
+
image::2026_spring/module-02-mtv/10a_Mig_Plan_Net.png[link=self, window=blank, width=100%]

. On the next screen you will be tasked with providing storage details for your migration plan. Select the radio button to *Use a new storage map*, then from the Source storage dropdowns make sure that the two workload shares from the VMware vSphere environment are listed, and from the Target storage dropdowns the default storage class *ocs-external-storagecluster-ceph-rbd* are selected for both. Finally, fill in the Storage map name with *default-storage-map* and click the *Next* button.
+
